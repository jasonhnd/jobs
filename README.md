...
厚生労働省の **職業情報提供サイト（job tag）** / JILPT IPD v7.00 に登録されている **556 の日本の職業** について、年収・学歴・就業者数・将来性などの構造化データに加えて、**複数のAIによる総合（AIOIS-10）の AI 影響度（Transformation）と仕事が減るリスク（Displacement-Risk）**、およびその理由を重ねた可視化サイトです。公開値は、3社のAIそれぞれの最新モデルによる採点の平均。個別モデルの採点は [モデル比較](https://mirai-shigoto.com/models) と履歴に残します。UI は日本語で、国内の読者を対象としています。
...
3. **モデル** — 各 run のモデル ID・provider・採点日は `data/scores/*.json` に記録します。公開値は、3社のAIそれぞれの最新モデルによる採点の平均。個別モデルの採点は /models と履歴に残す。別モデルへの silent fallback は行いません。
...
- **face validity 以上の妥当性は主張しません。** 実務者へのフォローアップ調査も、実際の代替率との比較も、信頼区間もありません。履歴上は 2026-06-23 に、当時の正典だった Claude Fable 5 batch（2026-06-13）の代表 40 職業を Claude Opus 4.8 / Sonnet 4.6 と比較しましたが、これは Fable predecessor の外部整合性チェックであり、現行の公開総合値の全量検証ではありません。公開値は、3社のAIそれぞれの最新モデルによる採点の平均です。
...
TypeScript ETL（`src/data/build.ts`）が MHLW jobtag の政府公開データを取り込み、複数のAIによる総合（AIOIS-10）の AI 影響度 / 仕事が減るリスクと結合し、Zod スキーマ（`src/data/schema/*.ts`）で検証した上で、`public/` 配下に projection を書き出します。続いて Astro が `src/pages/` を静的レンダリングし、結果の `dist-astro/` を Vercel がデプロイします。パイプライン全体は `bun run build` で実行できます。
