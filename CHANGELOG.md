...
- `ai-gateway` scoring provider (#340): any vendor's model through Vercel
  ...
  differs from the request fails the attempt (`model_mismatch`), and
  accepted responses are attested per-occupation in
  `<runDir>/executed-models.jsonl`. `check:score-batch` verifies that
  sidecar when present. The pre-batch gate banner in
  `docs/SCORING_RUNBOOK.md` is fulfilled and removed.
