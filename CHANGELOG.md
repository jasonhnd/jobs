...
- `/haid` — HAID v1.0 (人類と AI の距離 10 段階) standard page: ten behaviourally
  defined levels in four relations (無縁 1–2 / 道具 3–6 / 同席 7–8 / 一体 9–10),
  three named relation boundaries, terms, nesting rule (N(≥k) / n(k)),
  measurement and reporting rules, certainty labels with drawing rules, v1.0
  boundary rulings, conformance levels, revision policy, CC BY 4.0. Definitions
  live in `src/site/haid-spec.ts` and are emitted as `data.haid-spec.json`
  (no counts). Quarterly counts follow on `/aiadoption` from 2026-Q3. Footer
  link, sitemap entry, OG card, `/data` row, `docs/HAID.md`. No inline script.
...
- Grok 4.6 scoring path (mms-7a) is in-agent — the running `grok-4.6`
  ...
  as the public 提供元. No bespoke xAI provider.
- `/models` hub and per-run titles keep Noto Serif JP (weight 700) but
  beat the site-wide 1.7rem cap: hub h1 is `clamp(2rem, 4.6vw, 4.2rem)`.
  The previous sans + weight-800 face is still gone.
...
- Vercel AI Gateway scoring provider (`ai-gateway`, #340 / #399). Scoring
  transports are `in-agent` (running session) and `codex` (local Codex
  CLI) only. `AI_GATEWAY_API_KEY` and the `executed-models.jsonl` sidecar
  check are gone. No batch scored through Gateway ever landed in
  `data/scores/`.

### Added

- Grok 4.6 in-agent batch lands as the 5th consensus vote (mms-7c /
  #387): `data/scores/occupations_grok-4.6_2026-09-07.json`, 556/556.
  Vote count 4→5. Site mean 4.68→4.73. `|Δ| ≥ 0.5` on 32 occupations;
  40 band changes; none `|Δ| ≥ 1.0`. `/models/grok-4.6` 308 to the run
  page. C-facing copy does not name the model.
...
- WAF rule `ratelimit-shindan-share` exceeded-action flipped log → deny(429)
  ...
  `docs/SCORING_RUNBOOK.md` gained a self-deleting pre-batch gate banner
  for Vercel AI Gateway (#340); that transport was later removed (#399).
