#!/usr/bin/env bun
/**
 * make-fullrun-chunks.ts — extract chunks for the full-run remainder (all
 * occupations EXCEPT the ids in an exclude manifest, normally the approved
 * pilot sample whose scores are carried unchanged).
 *
 * Same extract shape as the Opus run (extract-occ-chunks extractOcc); chunk
 * headers carry NO rubric — the frozen prompt named by --prompt-file is the
 * canon.
 *
 ...
 *     --exclude .cache/scoring/<model>/pilot/sample.json \
 *     [--chunk 10] [--out .cache/scoring/<model>/full/chunks]
 */
...
if (!promptFile) fail('--prompt-file <path> is required (the frozen rubric the chunk headers point at)');
const excludeArg = args['exclude'];
if (!excludeArg) fail('--exclude <path> is required (the pilot sample manifest whose ids are carried unchanged)');
const excludePath = resolve(ROOT, excludeArg);
const outDir = resolve(ROOT, args['out'] ?? '.cache/scoring/full/chunks');
...
chunks.forEach((ch, idx) => {
  ...
  const header =
    `# full-run chunk ${n}/${chunks.length} — ${ch.length} occupations (ids ${ch.map((o) => o.id).join(', ')})\n` +
