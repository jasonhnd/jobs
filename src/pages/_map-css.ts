...
export const MAP_PAGE_CSS = `
    ...
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* RA-138 — time slider UI (Phase 5a placeholder data). 2020 / 2026 / 2030
       triad lets users see past · current · forecast AI risk on the same
       treemap. 2020 and 2030 are demo estimates (current ai_risk ± 2 clamped
       to [1,10]) — clearly labeled as such in .ml-meta and .timeline-demo-warning. */
    .map-timeline {
    ...
    .ml-label { font-weight: 600; color: var(--fg2); margin-right: 4px; }
    .ml-year {
    ...
    .ml-year:hover { background: var(--bg3); border-color: var(--accent); }
    .ml-year:focus-visible { outline: 2px solid var(--accent); outline-offset: 2px; }
    .ml-year[aria-pressed="true"] {
      background: var(--accent-deep); color: var(--bg); border-color: var(--accent-deep);
    }
    .ml-year small { font-size: 0.66rem; font-weight: 400; opacity: 0.85; }
    .ml-meta {
      margin-left: auto; color: var(--accent);
      font-size: 0.72rem; font-weight: 600;
    }
    @media (max-width: 600px) {
      .ml-meta { margin-left: 0; flex-basis: 100%; text-align: left; }
      .map-timeline { padding: 8px; }
    }
    /* Warning banner appears next to legend when viewing 2020 or 2030. */
    .timeline-demo-warning {
      display: inline-block;
      padding: 4px 10px;
      margin: 0 4px 12px;
      background: rgba(217, 107, 61, 0.10);
      border: 1px solid var(--accent);
      border-radius: 6px;
      color: var(--accent);
      font-size: 0.76rem; font-weight: 600;
    }
    /* Top-delta highlight box shown when year = 2030. Listed top 5 jobs that
       (under the placeholder ±2 model) hit the ai_risk = 10 ceiling. */
    .map-delta-highlight {
      margin: 16px 4px 0;
      padding: 14px 16px;
      background: var(--bg2);
    ...
    .map-delta-highlight .dh-title {
      font-family: var(--font-serif);
      font-size: 0.92rem; font-weight: 600;
      color: var(--fg); margin: 0 0 8px;
    }
    .map-delta-highlight .dh-list {
      list-style: decimal inside; padding: 0; margin: 0;
      font-size: 0.84rem; color: var(--fg2);
      line-height: 1.7;
    }
    .map-delta-highlight .dh-list li { padding: 2px 0; }
    .map-delta-highlight .dh-list li strong {
      color: var(--fg); font-weight: 600;
    }
    .map-delta-highlight .dh-delta {
