...
const DETAIL_PAGE_SPECIFIC_CSS = `
    ...
    .meta-row .band-low,.meta-row .band-cool{background:var(--risk-pill-low-bg);color:var(--risk-pill-low-fg)}
    .meta-row .band-mid,.meta-row .band-warm{background:var(--risk-pill-mid-bg);color:var(--risk-pill-mid-fg)}
    .meta-row .band-high,.meta-row .band-hot{background:var(--risk-pill-high-bg);color:var(--risk-pill-high-fg)}
    ...
    .v-doors a.solid{background:var(--orange-hot);color:var(--paper);border:1.5px solid var(--orange-hot)}
    ...
    html body #wrapper .me-cta-strip{display:flex;flex-direction:row;margin:12px 0 0;padding:13px 15px;background:var(--bg3);border:1px solid var(--border);border-left:1px solid var(--border);border-radius:14px;flex-wrap:nowrap;align-items:center;gap:10px}
    html body #wrapper .me-cta-strip p{flex:1 1 auto;font-size:var(--t-sm);line-height:1.6}
    html body #wrapper .me-cta-strip a{flex:0 0 auto;min-height:44px;padding:10px 15px;white-space:nowrap}
    @media (max-width:600px){html body #wrapper .me-cta-strip{flex-direction:row;align-items:center}}

    /* Relocated type block — class names byte-identical for #324 */
    .type-block{margin:18px 0 8px;padding:18px;background:var(--paper);border:1px solid var(--line-strong);border-radius:8px;box-shadow:0 1px 0 rgba(0,0,0,0.03),0 10px 28px rgba(120,80,30,0.07)}
    @media (min-width:900px){.type-block{padding:26px 30px}}
    ...
    .verdict-task span{display:block;margin:0 0 6px;font-size:var(--t-xs);line-height:1.25;color:var(--green-deep);font-weight:700;letter-spacing:0}
    .verdict-task p{margin:0;font-size:var(--t-sm);line-height:1.65;color:var(--ink-2)}
    @media (min-width:900px){.verdict-task{padding:14px 15px}.verdict-task p{line-height:1.7}}
    .verdict-one-line{margin:14px 0 0;padding:12px 14px;background:var(--orange-soft);border-left:3px solid var(--orange);border-radius:0 8px 8px 0;font-size:var(--t-sm);line-height:1.65;color:var(--ink);font-weight:700}
    @media (min-width:900px){.verdict-one-line{margin-top:18px;padding:14px 16px}}

    /* Multi-model score history */
    section.score-history{margin-top:18px;line-break:strict}
    section.score-history > h2{margin-bottom:8px;overflow-wrap:break-word;word-break:normal}
    .score-history-note{max-width:var(--content-max);margin:0 0 12px;font-size:var(--t-sm);line-height:1.7;color:var(--ink-meta);overflow-wrap:break-word;word-break:normal}
    @media (min-width:900px){.score-history-note{margin-left:auto;margin-right:auto}}
    .score-history-note a{color:var(--orange-hot);text-decoration:underline;text-decoration-thickness:0.06em;text-underline-offset:0.16em}
    .score-history-note a:hover{text-decoration-thickness:0.08em}
    .score-history-current{max-width:var(--content-max);min-width:0;margin:0 0 12px;padding:16px 18px;background:var(--paper);border:1px solid var(--line-strong);border-left:4px solid var(--orange-hot);border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:14px;box-shadow:0 1px 0 rgba(0,0,0,0.03),0 6px 18px rgba(120,80,30,0.04)}
    ...
    .score-history-details summary{display:flex;align-items:center;min-height:40px;width:max-content;max-width:100%;padding:8px 12px;border:1px solid var(--line-strong);border-radius:8px;background:var(--cream-2);color:var(--ink);font-size:var(--t-sm);line-height:1.35;font-weight:700;cursor:pointer;overflow-wrap:break-word;word-break:normal}
    /* Design.md §2.2-1 — --orange-hot is 4.37:1 on --cream-2 and cannot be text
       there. The border keeps the hover affordance; --orange is legal on a rule
       (§2.2-5), just never on text. */
    .score-history-details summary:hover{border-color:var(--orange)}
    .score-history-details[open] summary{margin-bottom:10px}
    .score-history-aging{max-width:var(--content-max);margin:0 0 10px;font-size:var(--t-sm);line-height:1.7;color:var(--ink-meta);line-break:strict;overflow-wrap:anywhere;word-break:normal}
    ...
    .ai-fact{margin:0 0 18px;padding:16px 18px;background:var(--paper);border:1px solid var(--line-strong);border-left:4px solid var(--red);border-radius:0 12px 12px 0;font-size:var(--t-body);line-height:1.85;color:var(--ink);box-shadow:0 1px 0 rgba(0,0,0,0.03),0 6px 18px rgba(120,80,30,0.04)}
    @media (min-width:900px){.ai-fact{padding:20px 22px}}
    ...
    dl.stats dt{font-size:var(--t-xs);color:var(--ink-meta);text-transform:none;letter-spacing:normal;margin:6px 0 0;order:2;font-weight:600}
    @media (min-width:900px){dl.stats dt{margin:8px 0 0}}
    dl.stats dd{font-size:var(--t-h2);font-weight:700;color:var(--ink);letter-spacing:-0.02em;line-height:1.1;order:1;word-break:keep-all;overflow-wrap:anywhere}
    @media (min-width:900px){}
    ...
    dl.stats dd .stat-empty{color:var(--ink-meta);font-weight:600;font-size:var(--t-sm);cursor:help;text-align:left;display:inline-block;vertical-align:middle}
    ...
    section.context p.definition::before{content:"TL;DR";color:var(--ink-meta);font-weight:700;flex-shrink:0;font-size:var(--t-xs);letter-spacing:0.06em;padding-top:3px}
    @media (min-width:900px){section.context p.definition{padding:14px 18px;margin:0 0 20px}}
    ...
    dl.radar-legend dt{font-family:var(--font-sans);font-size:var(--t-xs);color:var(--ink-meta);margin:0;text-transform:none;letter-spacing:normal;order:1;font-weight:400}
    dl.radar-legend dd{font-family:var(--font-sans);font-size:var(--t-h2);font-weight:700;color:var(--ink);font-variant-numeric:tabular-nums;margin:2px 0 0;order:2}
    @media (min-width:900px){}
    ...
    .transfer-card .tc-risk{font-family:var(--font-sans);font-weight:700;padding:3px 8px;border-radius:5px;font-size:var(--t-xs);letter-spacing:0.01em;background:var(--risk-pill-high-bg);color:var(--risk-pill-high-fg)}
    ...
    .faq-item summary::after{content:"+";position:absolute;right:16px;top:50%;transform:translateY(-50%);font-size:var(--t-h2);color:var(--ink-meta);transition:transform 200ms ease,color 200ms ease;font-weight:300}
    ...
    .org-list a{color:var(--orange-hot);text-decoration:none}
    ...
    .map-back-link a:hover,.map-back-link a:focus-visible{color:var(--orange-hot);border-color:var(--accent);background:color-mix(in srgb, var(--orange) 6%, transparent)}
    ...
    section.aiois10 > h2 a:hover{color:var(--orange-hot)}
    ...
    .aio-val{font-family:var(--font-sans);font-weight:700;font-size:var(--t-xs);color:var(--ink-2);text-align:right;font-variant-numeric:tabular-nums}
    .aio-note{margin-top:12px;font-size:var(--t-xs);color:var(--ink-meta);line-height:1.7}
    .aio-note .aio-tag{margin:0 1px 0 4px}
    .aio-note a{color:var(--orange-hot);text-decoration:underline;text-decoration-thickness:0.06em;text-underline-offset:0.16em}
