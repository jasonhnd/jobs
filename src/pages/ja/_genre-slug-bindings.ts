/**
 * src/pages/ja/_genre-slug-bindings.ts — single-entry binding builder
...
export interface GenreSlugInput {
  ...
  /** URL prefix segment, e.g. 'abilities' for /ja/abilities/{slug}. */
...
export function buildGenreSlugBindings(input: GenreSlugInput): GenreSlugBindings {
  ...
  const canonical = `${SITE_ORIGIN}/ja/${genrePath}/${cfg.slug}`;
  ...
  const relatedHtml = '<ul class="related-genre">' + allConfigs
    ...
    .map((m) => `<li><a href="/ja/${genrePath}/${m.slug}"><span class="rg-name">${escapeHtml(m.short_ja)}</span><span class="rg-desc">${escapeHtml(m.description_ja.slice(0, 60))}…</span></a></li>`)
    .join('') + '</ul>';
