...
export const QA_ITEMS: ReadonlyArray<QAItem> = [
  ...
  },
  {
    ...
    reasoning: '医師・看護師・薬剤師・弁護士など業務独占資格を要する職業は、法的に AI が直接代替することができません。これは強力な参入のかべです。一方、業務の中身 (リサーチ・書類作成・診断補助等) は AI で大幅に効率化されます。「資格があれば安泰」ではなく「資格 + AI を使いこなすスキル」が今後の優位性になります。',
    selector: (d) => {
  ...
  },
  {
    ...
    reasoning: '40 代の転職は、(1) 蓄積した専門性で評価される分野、(2) 60 代まで続けられる業務、(3) AI 化が進みにくい本質的価値を出せる分野、を重視すべき。具体的には士業 (弁護士・公認会計士)・建設管理・専門医・指導職等。AI 影響 大の事務系から低 AI の対人・現場系への横展開も選択肢ですが、年収ダウンを覚悟する場面が多い。',
    selector: (d) => {
      const age = d.stats?.average_age;
      const ai = d.ai_risk?.score ?? 99;
