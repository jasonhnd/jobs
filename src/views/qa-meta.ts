...
export const QA_ITEMS: ReadonlyArray<QAItem> = [
  // ── AI 不安系 (8) ──
  {
    ...
    reasoning: 'AI が苦手とするのは、(1) 物理的な世界での即興的判断 (建設現場・救急処置)、(2) 人間の感情を読み取り信頼関係を築く対人スキル (看護・介護・保育)、(3) 例外的な状況への適応 (緊急救急)。これらが業務の中核になっている職業ほど AI 影響度が低くなる傾向にあります。本サイトの AI 影響度は Claude Opus 4.8 による独自分析 (非公式) で、職業選択の参考情報の一つとしてご活用ください。',
    selector: lowAi,
