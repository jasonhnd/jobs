...
export const FAQS: Record<RankingSlug, ReadonlyArray<readonly [string, string]>> = {
  ...
  'ai-risk-low': [
    ...
    ['AI影響度が低い仕事は将来安心ですか？', 'AI代替リスクは低いですが、少子高齢化や産業構造変化など他の要因も考慮が必要です。'],
  ...
  salary: [
    ...
    ['高年収の職業に共通する特徴は？', '高度な資格・長期の教育投資・強い参入のかべのいずれかを持つ職業が多い傾向にあります。'],
  ...
  'recruit-ratio': [
    ...
    ['求人倍率と年収の関係は？', '人手不足が深刻な業種では時給・賃金の上昇圧力が強くなる傾向ですが、厳しさがその一因のことも多いため労働条件全般の確認が必要です。'],
  ...
  'recruit-ratio-low': [
    ...
    ['なぜ求人倍率が低い職業が存在する？', '人気職業 (クリエイティブ系・士業など)、参入のかべが高い職業、または市場規模が縮小傾向の職業で求人倍率が低くなる傾向です。'],
  ...
  'ai-replaced-soon': [
    ...
    ['AI 影響度はどう測定された？', 'Claude Opus 4.8 が IPD タスク情報を基にタスクレベル代替可能性を 0-10 で評価。本サイトの独自分析 (非公式) です。'],
  ],
  'ai-resistant-craft': [
  ...
  'ai-safe-young-workforce': [
    ['若手中心 × AI 安全な職業の特徴は？', 'IT 補助系・サービス系・現場系で平均年齢が低く、対人や身体性で AI への強さも持つ職業群です。'],
  ...
  'high-salary-young-entry': [
    ...
    ['初任給が高ければ将来も安心？', '初任給は出発点。年収カーブと AI 影響度を併せて確認しないと長期的な評価はできません。'],
    ['新卒で狙うべき分野は？', 'IT 系は AI 影響度が高めの面もありますが、AI を使いこなす側に立てば優位性を持続できます。'],
  ],

  // ── Phase 2: 教育・資格軸 (5) ──
  'license-required': [
    ['国家資格が必要な職業の特徴は？', '医療・士業・建設・福祉・教育系の専門職で、参入のかべが明確に設定されている職業群です。'],
    ['資格があれば AI に強い？', '法的に「資格保有者しかできない」業務範囲があるため、参入のかべ的に AI 代替が起きにくい傾向があります。ただし業務内訳の AI 化は別途進行します。'],
  ...
  'graduate-school-required': [
    ...
    ['大学院卒中心職に向く人は？', '長期の探究を嫌がらず、抽象概念や複雑な体系に取り組むことを楽しめる人。学位は通過点で、その先の研究実績が本番です。'],
  ],
  ...
  'self-employed-typical': [
    ...
    ['独立を目指すならどう準備する？', '現職での技能・人脈・顧客の蓄積、副業ステップ、開業に必要な資金 200-1000 万円帯の準備が定番です。'],
  ],
  ...
  'regulated-protected': [
    ['規制で守られた職業とは？', '関連資格が複数必要かつ AI 影響度も低い、参入のかべと AI への強さの両方を備えた職業群です。'],
    ['規制保護があれば安心？', '法的に守られている業務範囲がある一方、規制緩和や業務内訳の自動化は時間軸で進行します。職業自体ではなく業務再設計が課題。'],
