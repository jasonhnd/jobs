...
/** 数字の出どころと計算 — one card per level: 使った数字 → 計算 → 結果. */
export const HAID_RELEASE_PROVENANCE_JA = {
  heading: '数字の出どころと計算',
  intro: '各段階の「第 k 段階以上にいる人数」N(≥k) を、公開されている数字からどう出したか。段階ごとに、使った数字 → 計算 → 結果 の順に示します。式の中の数字は公表値そのままで、丸めるのは表示のときだけです。',
  /** Rules that apply to every level. Level-specific rules live in each card's 計算. */
  rules: [
  ...
  colCalc: '計算',
  colResult: '結果',
  /** how: one sentence per method, shown at the top of 計算 */
  howSingle: '公表値が 1 つなので、そのまま使う。',
  howMaxSingle: '{count} 件の公表値は重なりが分からないため、最大の 1 社を下限とする。',
  howSumRange: '各社の値を足し、重なり率を引いて中とする。最大の 1 社が低、単純合計が高。',
  howMarketUnion: '市場ごとに積み上げ、独立した上からの推計（人口 × 利用率）と突き合わせる。',
  howNone: 'この段階以上の人数を示す公表値がない。',
  /** steps: label + text */
  stepPicked: '最大の 1 社',
  stepPickedText: '{term} {max}',
  stepMarketUnionText: '{unionTerm} の重複除去済み合計をそのまま使う → {union}',
  stepMarketSumText: '{count} 社を足すと {sum}。重なり率 {ratePct} を引いて {union}',
  stepSumText: '{count} 社を足すと {sum}。重なり率 {ratePct} を引いて {mid}',
  stepBottomUp: '積み上げ',
  stepBottomUpText: '{parts} = {bottomUp}',
  stepTopDown: '上から',
  stepTopDownText: '{baseLabel} {base} × 利用率 {share} = {topDown}（{term}）',
  stepRawSum: '参考',
  stepRawSumText: '重なりを引かない単純合計は {rawSum}',
  /** range line under 計算 */
  rangeLow: '低',
  rangeHigh: '高',
  rangeMid: '中',
  rangeMidGeo: '中 = √(低 × 高)',
  rangeMidOverlap: '中 = 高 × (1 − 重なり率)',
  rangeFromBottomUp: '積み上げ',
  rangeFromTopDown: '上から',
  rangeFromMax: '最大の 1 社',
  rangeFromSum: '単純合計',
  /** input flags (chips) */
  flagStale: '古い',
  flagWeekly: '7 日口径',
  ...
  marketWorld: '世界',
  /** notes under 結果 */
  noteStale: '{term} は 12 か月より古い。',
  noteWeekly: '{term} は 7 日口径。30 日口径ではこれ以上。',
  noteFloored: '計算値 {computed} は N(≥{next}) = {nextValue} を下回るため、{nextValue} に合わせた。',
  noteFlooredNone: '公表値はないが、N(≥{next}) = {nextValue} を下回れないため、{nextValue} に合わせた。',
  none: '公表値なし',
  levelOne: '第 {k} 段階',
  levelSpan: '第 {from}〜{to} 段階',
  atLeast: 'N(≥{k})',
  exactly: 'ちょうどこの段階 n({k}) = {a} − {b} = {n}',
} as const;
