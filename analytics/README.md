...
`--check` は双方向で報告する。spec にあってプロパティに無いものは未適用の変更なので **exit 1**。プロパティにあって spec に無いものは報告のみ(GA4 標準の key event が常に該当する)。ただしその報告は重要で、`setup` は作成しかしない = 機能が廃止されても dimension と key event はプロパティに残り続ける。#240 の死んだ dimension 9 個はそうやって溜まった。
