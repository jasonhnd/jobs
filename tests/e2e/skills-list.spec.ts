/**
 * skills-list.spec.ts — skills list-first first screen (#328 family 7).
 *
 * Rows keep the `.skill-score` extra. This spec pins reorder:
 * H1 + sub + rows on the first screen; intro / スキルとは / stats folded.
 * Skills pages render no `.ai-fact`.
 */
...
test('390×844 first screen shows H1, sub, and rows; prose is folded', async ({ page }) => {
  await openConsentDecided(page, '/skills/critical-thinking', 390, 844);
  ...
  await expect(rowLink.locator('.skill-score')).toBeVisible();
  ...
  await expect(page.locator('.skill-detail')).toBeHidden();
});

test('folded chapter opens on desktop helper at 1280 and keeps intro + スキルとは', async ({ page }) => {
  await openConsentDecided(page, '/skills/critical-thinking', 1280, 800);
  const chap = page.locator('details.chap');
  await expect(chap).toHaveCount(1);
  await expect(chap).toHaveAttribute('open', '');
  await expect(chap.locator('.intro')).toBeVisible();
  await expect(chap.locator('.skill-detail')).toBeVisible();
  await expect(chap.locator('.stats')).toBeVisible();
  const wordBreak = await chap.locator('.skill-detail-grid li').first().evaluate(
