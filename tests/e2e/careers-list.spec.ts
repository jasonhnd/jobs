/**
 * careers-list.spec.ts — careers list-first first screen (#328 family 4).
 *
 * Row atom inherited from Hub.ts (#328 family 3). This spec pins reorder:
 * H1 + sub + rows on the first screen; intro / 特徴 / stats folded.
 * Careers pages render no `.ai-fact`.
 */
...
test('folded chapter opens on desktop helper at 1280 and keeps intro + 特徴', async ({ page }) => {
  ...
  const wordBreak = await chap.locator('.genre-detail-grid li').first().evaluate(
