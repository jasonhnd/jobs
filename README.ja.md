...
![Japan Jobs × AI Risk — 厚労省 jobtag に基づく日本の 552 職業の treemap に LLM による AI 代替リスクスコアを重ねたバイリンガル可視化サイト](og.png)

厚生労働省の **職業情報提供サイト（job tag）** に登録されている **552 の日本の職業** について、年収・学歴・就業者数・将来性などの構造化データに加えて、**LLM が 0〜10 で採点した AI 代替リスクスコア** とその理由（日本語・英語）を重ねた、バイリンガル可視化サイトです。
...
データの土台は、公開されている日本政府統計（job tag、労働力調査、経済センサス）です。リスクスコアは別途 LLM が生成しており、調査結果ではなく **モデル出力であることを明記** しています。UI は意図的にバイリンガル — 同じデータセットに 2 つの顔を持たせ、日本語は国内向け、English は海外向け、右上のトグルで切り替えられます。
...
- **色覚配慮（viridis）** トグル。
- **ライト / ダークモード** — 初回訪問は `prefers-color-scheme` で自動判定、太陽 / 月のトグルで明示的に切り替えると `localStorage` で記憶します。
- **バイリンガル UI** — 日本語 / English、ブラウザロケール自動判定。すべてのラベル、tooltip、詳細ページが両言語で存在します。
- **職業名でリアルタイム検索**（日英どちらでも）。
- **タイルにマウスオーバー（PC）またはタップ（スマホ）** で tooltip — リスクスコア、年収、就業者数、LLM の評価理由を表示。
- **552 職業の専用詳細ページ** — `/ja/<id>.html` と `/en/<id>.html`、各ページに評価理由のフルテキスト、年収 / 平均年齢 / 労働時間 / 求人倍率 / 学歴の内訳、検索エンジン向けの構造化データ（Schema.org `Occupation` JSON-LD）を含む。
...
日本にも BLS OOH に相当する公的データが存在します — **厚生労働省 職業情報提供サイト（job tag）**、約 500 職業、年収・学歴・就業者数・将来性などが構造化された形で提供されています。このデータセットは数年前から存在していたにもかかわらず、Karpathy 流の treatment（政府データに基づき、バイリンガルで、LLM スコア付き）を施したものは **誰も作っていませんでした**。

本プロジェクトはその欠けていたピースです。Karpathy のアイデアを日本に移植し、同じデータセットを 2 言語で読めるよう翻訳ステップを 1 つ加えた、それだけです。
...
スコアリングは `scripts/make_prompt.py` でビルドした日英プロンプトバンドルを Claude Code セッションに渡して実行します。各職業について：
...
4. **出力** — モデルのスコア + バイリンガル理由文、職業ごとにキャッシュ。再実行時は既出力の職業はスキップ。
...
- **日英の理由文は乖離しうる。** 同一の呼び出し・同一の context から生成されますが、モデルが言語ごとに強調する側面を変えることが時々あります。これは特長（各言語の読者が自然な散文を読める）であり、限界（厳密な対訳ではない）でもあります。
...
[karpathy/jobs](https://github.com/karpathy/jobs) の構造を日本向けにローカライズし、翻訳ステップを 1 つ追加して、すべての出力をバイリンガル化したものです。

| # | スクリプト | 役割 | 出力 |
| --- | --- | --- | --- |
| 1 | `scripts/list_occupations.py` | job tag の A〜Z インデックスからマスター職業リストを生成 | `occupations.json` |
| 2 | `scripts/scrape_jobtag.py` | 各職業ページを礼儀正しいレートで取得（httpx 優先、Imperva 防御ページは Playwright フォールバック） | `html/<slug>.html` |
...
| 5 | `scripts/translate.py` | LLM による日→英バッチ翻訳（職業名・業種・記述） | 同 CSV にバイリンガル列をマージ |
| 6 | `scripts/make_prompt.py` + Claude Code セッション | 0〜10 の AI 代替リスクスコア + 日英理由文を LLM が生成（[算出方法](#ai-代替リスクスコアの算出方法) 参照） | `data/scores/occupations_<model>_<date>.json` (ScoreRun v2.0 schema) |
...
| 8 | `scripts/build_occupations.py` | `dist/data.detail/<id>.json` から 1,112 件の静的詳細ページ（日 + 英、声優・ブロックチェーン・エンジニア等の 4 新規職業を含む）を生成 | `ja/<id>.html`, `en/<id>.html` |
...
| SEO / GEO | `robots.txt` で 17 種の LLM クローラーを明示許可、`sitemap.xml` が 1,104 URL を hreflang ペア付きで網羅、[`/llms.txt`](https://mirai-shigoto.com/llms.txt) は [llmstxt.org](https://llmstxt.org/) 標準準拠、Schema.org `WebSite` + `Dataset` + `Person` グラフを `<head>` に埋め込み。 | Google 検索と AI 検索の両方を意識した構成。 |
| テーマ | ライト / ダーク（`prefers-color-scheme` 検出 + 手動トグル） | フラッシュ防止のインライン初期化スクリプト。`localStorage` 永続化。テーマ切替で treemap 配色とキャンバス背景も同期。 |
...
uv run python scripts/make_prompt.py     # → AI scoring via Claude Code session
...
├── ja/<id>.html           # 556 職業詳細ページ（日本語、声優・ブロックチェーン・エンジニア等の 4 新規職業を含む）
├── en/<id>.html           # 556 職業詳細ページ（英語）
├── api/
│   ├── og.tsx             # Edge Function — 職業ごと 1200×630 OG 画像を動的生成
...
│   ├── translations/en/<id>.json # 552 件の英訳
│   ├── scores/                    # AI 採点ラン履歴（永不削除）
...
│   └── data.labels/{ja,en}.json  # フラットラベル辞書
├── og.png                 # 1200×630 ソーシャルカード（既定; 職業別は api/og）
├── robots.txt             # 主要 LLM クローラーを明示許可
├── sitemap.xml            # 1,112 URL を hreflang ペア付きで網羅
...
**データセットを引用する** 場合は、就業者数 / 年収 / 学歴などの数値については 厚生労働省（job tag）を、AI リスクスコア + バイリンガル提示レイヤーについて本プロジェクトをクレジットしていただければと思います。
...
- **Issues** — 手法に関する質問、データソースの提案、キャリブレーションへのフィードバック（「職業 X のスコアはおかしい — その理由は…」）、翻訳改善（特に学歴区分や業種境界の日本語用語）。
- **PR** — バグ修正、翻訳改善、新しい色レイヤー、アクセシビリティ改善。視覚 / レスポンシブ変更は [`docs/Design.md`](docs/Design.md) を先に更新する必要があります — 仕様書側がソースです。
...
- **[`/privacy`](https://mirai-shigoto.com/privacy)** — バイリンガルプライバシーポリシー（APPI + GDPR 対応）。
