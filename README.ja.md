...
スコアリングは `scripts/score_ai_risk.py` で実行されます。各職業について：
...
5. **集約** — `scripts/build_data.py` が CSV とスコアをマージし、フロントエンドが読む唯一のアーティファクト `data.json` を出力。
...
| 6 | `scripts/score_ai_risk.py` | 0〜10 の AI 代替リスクスコア + 日英理由文を LLM が生成（[算出方法](#ai-代替リスクスコアの算出方法) 参照） | `scores.json` |
| 7 | `scripts/build_data.py` | CSV とスコアをマージしてバイリンガルの単一アーティファクトを出力 | `data.json` |
| 8 | `scripts/build_occupations.py` | `data.json` から 1,104 件の静的詳細ページ（日 + 英）を生成 | `ja/<id>.html`, `en/<id>.html` |
...
| 視覚仕様 | [`Design.md`](Design.md) | デザイントークン、テーマシステム、ブレークポイント、treemap 描画ルール、tooltip 規定、コンポーネント標準を一括定義した単一真相源。視覚変更はこのファイルを先に更新し、コードを追従させる。 |
...
- **PR** — バグ修正、翻訳改善、新しい色レイヤー、アクセシビリティ改善。視覚 / レスポンシブ変更は [`Design.md`](Design.md) を先に更新する必要があります — 仕様書側がソースです。
...
- **[Design.md](Design.md)** — 視覚仕様：デザイントークン、テーマシステム、レスポンシブブレークポイント、treemap ルール、コンポーネント標準。
