...
| 視覚仕様 | [`docs/Design.md`](docs/Design.md) — デザイントークンとテーマの単一真相源 |
...
サイトは静的な `index.html` 1 枚 + `dist/` のビルド済みデータで構成されています。ローカルで閲覧するだけなら：
...
ビルドパイプラインの再実行は [`scripts/README.md`](scripts/README.md) を参照してください。[uv](https://docs.astral.sh/uv/) と LLM API キーが必要です。
...
├── docs/                   # 仕様書（Design.md）+ アーカイブ CHANGELOG
├── vercel.json             # 静的サイト設定
├── CHANGELOG.md            # リリース履歴
