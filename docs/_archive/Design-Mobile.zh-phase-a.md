...
> 引用 Design.md 的章节,Phase A (2026-05-13) 已对齐 §0.2 / §2 / §3 / §18;§5 / §6.1 / §7 部分仍待 Gen-3 审计。读者在引用任何 Design.md 章节前请先看 [Design.md §0.2 当前代际状态](./Design.md#02-当前代际状态)。

| 维度 | 在哪 | 状态 |
|---|---|---|
| 颜色 / 字体 / 间距 token | Design.md §2 | 🟢 LIVE (Phase A 已重写为两层语义+alias) |
| 主题系统 | Design.md §3 | ⚠️ NEUTRALIZED — 单主题,`data-theme` no-op |
| 响应式断点定义 | Design.md §4 | 🟢 LIVE |
| Treemap 视觉化(颜色函数 / 透明度 / 标签门槛 / GAP) | Design.md §5 | ⚠️ Dark 配色函数 deprecated,Light 路径 LIVE,完整审计待 Phase B |
| Desktop tooltip (hover-mode) | Design.md §6.1 | ⚠️ 待审计 — treemap 已转 `/map` 页,desktop hover tooltip 是否仍活需核实 |
| Tooltip 视口溢出处理(共享) | Design.md §6.4 | 🟢 LIVE(共享逻辑) |
| 通用组件 (top banner / stats panel / footer / 404 等) | Design.md §7.1-§7.10, §7.13, §7.14 | 🟢 LIVE — 其中 §7.10 footer (v1.3.x)、§7.12 desktop hero、§7.13 follow+share、§7.14 404 是最新最干净的契约 |
| Desktop Hero | Design.md §7.12 | 🟢 LIVE |
| 交互动效 / 可访问性 / 资产 / palette 准则 | Design.md §9-§13 | 🟢 LIVE |
| **样式架构契约(CSS 归属规则 + `_id-css.ts` pattern + 迁移状态)** | **Design.md §18** | **🟢 LIVE (Phase A 新增)** |

| 维度 | 在哪 | 状态 |
|---|---|---|
| Mobile Hero (Variant C) | 本文件 §1 | 🟢 LIVE |
| Mobile Tooltip 行为(touch-mode + close button + CTA + touch state machine) | 本文件 §2 | 🟢 LIVE |
| 移动端响应式规则汇总(≤768 / ≤480 / ≤360 / ≤540) | 本文件 §3 | 🟢 LIVE |
| `/map` 独立页规范(含 §4.4.1 重渲染契约) | 本文件 §4 | 🟢 LIVE |
...
| 2026-05-13 | §0.1 引用表 | Phase A 文档对齐(配合 Design.md §0.2 / §2 / §3 / §18 重写) | 引用表逐行加 🟢 LIVE / ⚠️ 待审计 / NEUTRALIZED 状态标记,把"Design.md 哪些章节仍可信任 / 哪些是 Gen 1 化石待审计"显式标出;新增 Design.md §18 样式架构契约的引用行。不动本文件 mobile 专属 §1-§5 内容(均已 LIVE)。零代码改动。|
