...
### 0.2 当前代际状态(2026-05-13 Phase A 引入)

设计在过去 12 个月里经历了三代演化。**规范长期只描述了 Gen 1**,因此 §2 颜色 token、§3 主题系统等章节与代码实际跑的东西不符。本节列出"当前 truth"与"历史 / 待审计"的分界,后续章节按此标记 🟢 LIVE / ⚠️ 待 Gen-3 审计 / 🔴 DEPRECATED。

| 代 | 时间 | 视觉方向 | 命名 | 当前去留 |
|---|---|---|---|---|
| Gen 1 | v0.4.x – v1.0.x | Dark + Light 双主题,中性灰 + 琥珀 `#ffb84d` | `--bg/--fg/--accent` 短名 | 🔴 DEPRECATED。仅 §15.LEGACY 保留备查 |
| Gen 2 | v1.1.0 – v1.2.0 | Direction C: Warm Editorial 单主题,cream + terracotta + sage | `--m-color-*` 长前缀 | 🔴 DEPRECATED。`styles/mobile-tokens.css` 无 consumer,待删 |
| Gen 3 | v1.2.0 – 当前 | 单主题 Warm Editorial + 语义化 token | `--cream/--ink/--orange/--green-deep` **正主** + `--bg/--fg/--accent` 保留为 alias | 🟢 LIVE。参考实现:[`src/pages/ja/_id-css.ts`](../src/pages/ja/_id-css.ts) |

| 维度 | 状态 |
|---|---|
| 视觉方向(配色 / 字体 / 编辑风格) | 🟢 LIVE: Gen 3 — Direction C: Warm Editorial 单主题 |
| 双主题切换 | ⚠️ NEUTRALIZED:`data-theme` no-op、切换按钮 `display:none`,详见 §3 |
| Token 命名 | 🟢 LIVE:两层结构 — 语义 + alias,详见 §2.1 |
| 旧 Gen 1 dark+amber token / 1400px wrapper / 系统字体规范 | 🔴 DEPRECATED,具体差异列在本节下表 |
| 旧 Gen 2 `--m-*` 前缀 token | 🔴 DEPRECATED — `styles/mobile-tokens.css` 无 import,见 §18.3 |
| 样式归属架构(谁来声明 token、谁来组装) | 🟢 LIVE (Phase A 新增):见 §18 |
| §5 Treemap 视觉 | ⚠️ Dark 配色函数路径 deprecated,Light 路径 LIVE,完整审计待 Phase B |
| §6.1 Desktop hover tooltip | ⚠️ 待审计:treemap 已迁 `/map` 独立页,desktop hover tooltip 是否仍活需核实 |
| §7 通用组件 | 🟢 LIVE:§7.10 footer / §7.12 desktop hero / §7.13 follow+share / §7.14 404 是最新最干净的契约 |

| 实质差异(Gen 1 规范 vs Gen 3 实施) | Gen 1 规范声称 | Gen 3 实际跑 |
|---|---|---|
| 颜色 | `--bg: #0b0d10 (dark) / #fafafa (light)`,`--accent: #ffb84d/#d97706` | `--bg: #FAF6EE`,`--accent: #D96B3D`(单主题) |
| 字体 | "系统字体优先,不引入 web font" | Google Fonts Noto Serif JP + Plus Jakarta Sans + Hiragino Mincho |
| Wrapper max-width | desktop 1400px | desktop 1080px |
| 卡片 radius | 8px(`--radius-card`) | 12-14px(transfer / topn / radar) |
| 主题切换 | 三态(system / light / dark)+ 可见 toggle | 单主题 + toggle `display:none` |

> 后续 §2-§7 各节将逐步标记状态。读者在引用本规范前请先看本节的状态表,**不要直接套用未带 🟢 LIVE 标记的旧值**。
...
**状态:** 🟢 LIVE (Gen 3) — 2026-05-13 Phase A 重写。Token 由两层组成:语义色板 (primary) + alias 短名 (legacy + 跨页通用)。

### 2.1 颜色

#### 第一层 — 语义色板(primary,新代码应使用)

命名表达颜色的语义角色,而不是布局角色。任何新组件 CSS **只引第一层**。

| Token | 值 | 用途 |
|---|---|---|
| `--cream`        | `#FAF6EE` | 主画布、页面背景 |
| `--cream-2`      | `#F2EADB` | 次级 surface、TL;DR 卡底 |
| `--paper`        | `#FFFFFF` | 浮出表面 — radar / FAQ / transfer / topn / ai-risk-detail 卡 |
| `--ink`          | `#241E18` | 主文本、标题 |
| `--ink-2`        | `#5a4a3a` | 正文段落、definition |
| `--ink-3`        | `#8a7a6a` | 三级文本、breadcrumb、副信息、tc-meta |
| `--ink-4`        | `#b0a090` | 占位 / aria 间隔符 |
| `--orange`       | `#D96B3D` | 主 accent、CTA、risk-high pill、risk 7-8 数字 |
| `--orange-hot`   | `#c0411e` | section h2、topn h3、risk 9-10 数字、faq b 强调 |
| `--orange-soft`  | `#fce4d2` | one-line callout 底色 |
| `--green`        | `#5fa050` | sector chip 底色、risk 0-2 数字 |
| `--green-deep`   | `#48705F` | 标题暖底配色、topn h3 在 transfer 段、ai-task h3 |
| `--red`          | `#c95a3a` | risk 9-10 数字最深档 |
| `--purple`       | `#8b5fb0` | 储备色,目前 [`_id-css.ts`](../src/pages/ja/_id-css.ts) 引入但未投入使用 |
| `--purple-soft`  | `#ddd5fb` | 储备 |
| `--line`         | `rgba(36,30,24,0.06)`  | 细分隔线(dotted、faq-answer border-top) |
| `--line-strong`  | `rgba(36,30,24,0.12)`  | 强分隔线、org-cert-block border、map-back-link border |

#### 第二层 — Alias 短名(legacy + 跨页通用)

为兼容 Gen 2 时期 16 个 hub 页内联 token + footer partial + render-function selectors,以下 alias 在 `_id-css.ts` 与未来的 `canonical-tokens.css` 中**永远**镜像到第一层(或保留为历史定值)。

| Alias | 等价于 / 值 | 用途 |
|---|---|---|
| `--bg`          | `--cream` (`#FAF6EE`)            | body 背景 |
| `--bg2`         | `--paper` (`#FFFFFF`)            | 卡片 / surface |
| `--bg3`         | `--cream-2` (`#F2EADB`)          | top-banner / 凹陷区 |
| `--fg`          | `--ink` (`#241E18`)              | 主文本 |
| `--fg2`         | `#7A6F5E`                        | 次文本 — **历史值,不严格等于 `--ink-2`** |
| `--fg3`         | `#A39785`                        | 三级文本 — 历史值,不严格等于 `--ink-3` |
| `--accent`      | `--orange` (`#D96B3D`)           | 主 accent |
| `--accent-2`    | `#6E9B89`                        | sage — 历史值,与 `--green` 接近但 RGB 不同 |
| `--accent-deep` | `--green-deep` (`#48705F`)       | 标题暖底 |
| `--border`      | `rgba(36,30,24,0.10)`            | 默认 surface 边框 — 历史值,与 `--line-strong` 不同 |

> **关键警告**:`--fg2 / --fg3 / --accent-2 / --border` 的具体 RGB 与"语义层第一行 / 类似名字的 token"**不严格相等**。这是 Gen 2 → Gen 3 过渡期遗留的微差,已部署在生产页面与 footer partial。**不要为了"看着整齐"统一掉**,会引起全站颜色微变。新组件应直接用第一层语义名,从源头避开 alias。

#### 编辑规则

1. 新组件 CSS:**只引第一层语义 token**(`--cream / --ink / --orange / --green-deep` 等)
2. 跨 hub 共用组件(footer / breadcrumb / 卡片基础 / canonical-css.ts footer):可引 alias(已部署 contract)
3. 改 alias 的等价映射:必须同时改本节表 + 全仓 grep 验证一致 + 跑 baseline diff (`pnpm run check:seo-baseline`)
4. 引入新色:先加到第一层,然后视必要补 alias
5. 所有 token 应由 `styles/canonical-tokens.css`(待建)统一提供,page-local CSS 只引用 token,不在 `:root{}` 重声明 — 详见 §18

#### Treemap 配色函数

> **状态:** ⚠️ Dark 路径 deprecated(`data-theme` neutralized,见 §3),Light 路径 LIVE。完整审计在 Phase B(检查 `greenRedCSSDark` 是否仍被引用 / 是否可删)。下表保留作为参考。

| 主题 | 函数 | t=0 锚点 | t=0.5 锚点 | t=1 锚点 |
|---|---|---|---|---|
| Dark(🔴 dead-code suspected) | `greenRedCSSDark` | `rgb(30, 180, 40)` 森林绿 | `rgb(230, 160, 20)` 琥珀 | `rgb(255, 30, 15)` 鲜红 |
| Light(🟢 LIVE) | `greenRedCSSLight` | `rgb(15, 195, 105)` 鲜翠 | `rgb(235, 115, 25)` 烧橘 | `rgb(235, 40, 55)` 鲜红 |
...
> Light 模式 alpha 必须高(≥0.85)— 白底会把低 alpha 颜色"洗白",破坏对比层级。

---

### 2.2 排版

**状态:** 🟢 LIVE (Gen 3) — Gen 1 的"系统字体优先,不引入 web font"已被推翻。
...
- macOS / Windows 优先用 Hiragino Mincho ProN / Yu Mincho(本地字体,零网络)
- 其余系统通过 BaseLayout 加载 Google Fonts(Noto Serif JP + Plus Jakarta Sans,400/500/600/700 各两族)
- BaseLayout 已 `<link rel="preconnect">` 到 `fonts.googleapis.com` / `fonts.gstatic.com`

#### 字号刻度(以 [`_id-css.ts`](../src/pages/ja/_id-css.ts) 为实施基准)

| 用法 | 值 | 实施位置 |
|---|---|---|
| h1 | `clamp(1.9rem, 1.5rem + 1.4vw, 3.2rem)`、weight 700、letter-spacing -0.015em、line-height 1.06 | `_id-css.ts:62` |
| h1 .h1-sub | 0.66em(相对于 h1)、color `--ink-3`、weight 500 | `_id-css.ts:64` |
...
| body 段落 | mobile 0.92rem / ≥900 1rem、line-height mobile 1.85 / ≥900 1.95、serif、color `--ink-2` | `_id-css.ts:131` |
| TL;DR / definition | mobile 0.85rem / ≥900 0.92rem、sans、weight 500 | `_id-css.ts:134` |
| breadcrumb | 0.74rem、sans、color `--ink-3` | `_id-css.ts:53` |
| meta-row | 0.78rem、color `--ink-3` | `_id-css.ts:67` |
| stat dd | mobile 1.2rem / ≥900 1.65rem、weight 800、letter-spacing -0.02em | `_id-css.ts:118` |
| stat dt | mobile 0.7rem / ≥900 0.78rem、color `--ink-3`、weight 400 | `_id-css.ts:116` |
| risk-card 数字 | mobile 2.4rem / ≥900 3.2rem、weight 900、letter-spacing -0.04em | `_id-css.ts:79` |
| topn item | 0.82rem | `_id-css.ts:164` |
| footer chip | 0.78rem(详见 §7.10) | partials/footer.html |
| footer-meta | 0.7rem(≤540: 0.66rem) | partials/footer.html |

#### Line-height

- 正文 / intro(serif):mobile 1.85 / ≥900 1.95
- 卡片 / 紧凑文本(sans):1.6-1.65
- 标题:1.06-1.45
...
**状态:** 🟢 LIVE (Gen 3) — 以 [`_id-css.ts`](../src/pages/ja/_id-css.ts) 为基准。Gen 1 的 `--radius-card: 8px` 已废弃。

| 维度 | mobile | ≥640 | ≥900(desktop) | 实施位置 |
|---|---|---|---|---|
| 卡片 padding | `14px 16px` (transfer/topn)、`16px 18px` (topn-block) | — | `20px 22px`(transfer)、`22px 24px`(stats)、`28px 32px`(narrow card) | `_id-css.ts:128,159,176,114` |
| section margin-top | 26px | — | 44px | `_id-css.ts:122` |
| stats grid gap | 10px | — | 16px | `_id-css.ts:111-113` |
| transfer-grid gap | 8px | 12px (≥768) | — | `_id-css.ts:173` |
| topn-grid gap | 14px | — | 18px | `_id-css.ts:157` |

| 圆角 | 值 | 用途 |
|---|---|---|
| 卡片 sm | 10px | one-line callout 右上 / definition / cert-list li |
| 卡片 md | 12px | transfer-card / topn-block-h3 box / org-cert-block / faq-item / 默认 |
| 卡片 lg | 14px | radar-wrap / ai-risk-detail / topn-block(≥900) / stats > div(≥900) / faq-item(继承) |
| risk-card | 14px | 主 risk 数字卡 |
| pill | 999px | sector-chip / band / map-back-link / footer chip |

| 阴影 | 值 | 用途 |
|---|---|---|
| 卡片浅 | `0 1px 0 rgba(0,0,0,0.03), 0 6px 18px rgba(120,80,30,0.04)` | radar-wrap / topn-block / ai-risk-detail / section.context 系列 / faq-item |
| transfer hover | `0 1px 0 rgba(0,0,0,0.03), 0 12px 28px rgba(217,107,61,0.10)` | transfer-card:hover/focus-visible |
| stats 卡 | `0 4px 12px rgba(120,80,30,0.04)` | dl.stats > div |
| risk-card | `0 4px 14px rgba(201,90,58,0.08)` | 主 risk 数字卡 |
| 404 CTA hover | `0 4px 14px rgba(217,107,61,0.28)`(详见 §7.14) | .cta-primary:hover |

#### 边框颜色

- 默认 surface:`--line-strong` (`rgba(36,30,24,0.12)`)
- 卡片轻 border:`rgba(0,0,0,0.04)` (transfer / topn / radar / ai-risk-detail / faq-item 等)
- transfer-card hover:`rgba(217,107,61,0.30)`(`--orange` 28% alpha)
- 强调 surface:**已不再使用 Gen 1 的"实心左条 3-4px"模式**;改为 `0 10px 10px 0`(one-line callout 左上方角变直,视觉上等价于左条)

---

### 2.4 布局尺寸

**状态:** 🟢 LIVE (Gen 3) — Gen 1 的 1400px wrapper 已废弃。
...
阅读区 narrow card max-width: 820px
  适用: section.context / section.how-to-become / section.working-conditions /
        ai-risk-detail / radar-wrap / faq-list
  在 desktop (≥900) 内 margin-left:auto / margin-right:auto 居中
```

> hub 页面(sectors / skills / compare / rankings 等 16 个)的 `#wrapper max-width: 980px` 是 Gen 2 历史值,与本节 1080px 不一致。Phase D 重构每页抽到 `_<name>-css.ts` 时统一为 1080px。

---

## 3. 主题系统

**状态(2026-05-13):** ⚠️ NEUTRALIZED — 自 Gen 2(v1.2.0)起,Direction C: Warm Editorial 是**单主题**。`data-theme` 属性 no-op,主题切换按钮在 [`_id-css.ts:45`](../src/pages/ja/_id-css.ts:45) 通过 `display: none !important` 隐藏。Gen 1 三态主题模型已 deprecated,本节描述当前实际行为 + 未来恢复 dark mode 的硬性条件。

### 3.1 实际行为(LIVE)

`prefers-color-scheme` 与 `[data-theme="light/dark"]` 选择器在 CSS 中**强制解析为同一套 cream 配色**,见 [`_id-css.ts:40`](../src/pages/ja/_id-css.ts:40):
...
效果:不管用户选什么,页面永远是 Warm Editorial cream。

### 3.2 保留的脚手架(LIVE,但无效果)

- [`BaseLayout.astro:75-84`](../src/layouts/BaseLayout.astro:75) 仍有 no-flash inline script 读 `localStorage.theme`,继续往 `<html>` 写 `data-theme` 属性 — 不影响显示,仅为未来 dark 变体保留
- 切换按钮 DOM 在部分页面仍存在,均被 `display: none !important` 隐藏(`.theme-toggle{display:none !important}`)
- GA4 `theme_change` 事件已**不再 fire**(无可见 UI 触发)

### 3.3 未来变体的硬性条件(PLANNED)

如果重启 dark mode,以下条件**不可绕过**:

1. 在 §2.1 第一层语义色板新增 `--cream-dark / --ink-on-dark / --orange-on-dark` 等暗色 token,**不**改 `--bg / --fg` alias 的值
2. 重新建立 `prefers-color-scheme: dark` 与 `[data-theme="dark"]` CSS rule 的 alias 映射(把 alias 在 dark 上下文里映到暗色 token)
3. Treemap canvas 重新启用 `greenRedCSSDark` 函数(目前疑似 dead code,见 §2.1)
4. 移除 `_id-css.ts:45` 的 `display: none`,GA4 `theme_change` 事件恢复 wiring
5. 所有页面 `_*-css.ts` 同步更新

### 3.4 切换按钮规格(🔴 隐藏,保留作为 Gen 1 历史)

(以下规格保留作为参考。当前**不渲染**;若 §3.3 路径走通后恢复,按此规格实施。)
...
| 2026-05-13 | §0.2 (新), §2 (重写), §3 (重写), §18 (新), Design-Mobile.md §0.1 | **Phase A 文档对齐 — 把规范从 Gen 1 描述更新到 Gen 3 实施现实** | 长期问题:Design.md §2 颜色 token 仍描述 Gen 1 (dark+light + amber `#ffb84d/#d97706`)、§3 三态主题模型、§2.2 "系统字体优先 不引入 web font"、§2.4 wrapper 1400px;但生产代码自 v1.2.0(Gen 2)起跑 Direction C: Warm Editorial 单主题(cream + terracotta),自 commit a9f6ed79(2026-05-13)起 page-specific CSS 在 `src/pages/ja/_id-css.ts` 以语义+alias 双层 token 实施 (Gen 3)。规范一直在描述 Gen 1,导致 16 个 hub 页面 / `_id-css.ts` / footer partial / mobile-tokens.css 共四套 token 各自漂移,用户层面表现为"网站看起来不那么统一"。**Phase A 工作:** (1) 新增 §0.2 当前代际状态,声明 Gen 1/2/3 哪一代是 LIVE,逐节标 🟢/⚠️/🔴 状态;(2) §2 完整重写为两层 token (语义 primary `--cream/--ink/--orange/--green-deep` + alias `--bg/--fg/--accent`),含 alias 与语义不严格相等的"历史值"警告;§2.2 字体栈更新为 Google Fonts + Hiragino Mincho 现状,§2.3 间距/圆角按 `_id-css.ts` 实施值重写,§2.4 wrapper 1400→1080;(3) §3 主题系统标 NEUTRALIZED,描述当前 `data-theme` no-op 行为 + 未来恢复 dark mode 的硬性条件;(4) 新增 §18 样式架构契约,描述 `_id-css.ts` page-local CSS 模式 + section CSS 导出(`RELATED_HUBS_CSS` / `SAME_RISK_CSS`) + 当前迁移状态;(5) Design-Mobile.md §0.1 引用表加 LIVE/⚠️/NEUTRALIZED 状态标记,把哪些 Design.md 章节仍可信任、哪些待审计明确化。**零代码改动**;Phase B(CI token 一致性守护)+ C(创建 `styles/canonical-tokens.css` + BaseLayout import)+ D(架构重构每个 hub 页抽到 `_*-css.ts` 时删 `:root` 复制块)待后续 PR 执行。|
...
## 18. 样式架构契约(与 architecture.md Layer 4 一致)

**状态:** 🟢 LIVE — 2026-05-13 Phase A 引入。本节描述 CSS 在代码结构中的归属规则,与 [architecture.md §2.4 Templates](./architecture.md) 一起读。

### 18.1 CSS 归属规则

| 内容 | 归属层 | 形态 | 例子 |
|---|---|---|---|
| 全站 token / reset / body / a / h-基础 | 全局共享 | `styles/canonical-tokens.css`(🆕 待建),BaseLayout 一次 import | — |
| 整页 page-specific CSS | page-local | `src/pages/<route>/_<route>-css.ts`,导出 `export const X_PAGE_CSS = \`...\`` | [`src/pages/ja/_id-css.ts`](../src/pages/ja/_id-css.ts) (commit a9f6ed79) |
| 跨页可复用的 section CSS | section-local | `src/data/lib/<section>-graph.ts` 同模块导出 `<SECTION>_CSS` 字符串 | [`spoke-hub-graph.ts:352`](../src/data/lib/spoke-hub-graph.ts:352) 导出 `RELATED_HUBS_CSS`;[`spoke-spoke-graph.ts:150`](../src/data/lib/spoke-spoke-graph.ts:150) 导出 `SAME_RISK_CSS` |
| 单 Astro 组件 scoped CSS | 组件 | `.astro` 文件内 `<style>` 块,被 Astro 自动 scope | `TopNav.astro` / `MobileNav.astro` / `Footer.astro` |

### 18.2 组装顺序

```
最终页面 <style> 内容
  = canonical-tokens.css                   ← 全站 token (BaseLayout 自动注入)
  + page-local _<page>-css.ts 的 X_PAGE_CSS
      ├─ 该页 page-specific 样式
      ├─ ${RELATED_HUBS_CSS}               ← section CSS,被 page CSS 拼接
...
- ❌ 不在 `.astro` 文件里写 `<style>` 含 ≥ 30 行 CSS。超过就抽到 `_<name>-css.ts`
- ❌ 不在 page-local `:root{...}` 中重复声明 token —— 所有 token 由 `canonical-tokens.css` 提供。page-local CSS 只能**引用** token (`var(--cream)`),不能**声明** token
- ❌ 不在 view / graph 层(`src/views/` / `src/graph/`)写 CSS 字符串
  - 唯一例外:`src/data/lib/*-graph.ts` 在迁移期允许导出 section CSS — 它们是 template 的 sibling,即 architecture.md §2.4 Templates 层的另一种形态(TS 函数通过 `trustedCss` 出口)
- ❌ 不在新代码中 import `styles/mobile-tokens.css` — 该文件已 DEPRECATED,无 consumer

### 18.4 当前迁移状态(2026-05-13)

| 文件 | 现状 | 目标 |
|---|---|---|
| [`src/pages/ja/[id].astro`](../src/pages/ja/[id].astro) | ✅ 已抽到 [`_id-css.ts`](../src/pages/ja/_id-css.ts) (commit a9f6ed79) | — |
| 16 个 hub 页面 (sectors / skills / abilities / knowledge / values / work-styles / training / life-balance / education / employment-types / entry-paths / licenses / careers / rankings / compare / explore / interests / q / yearly) | ⏳ 内联 `<style slot="head" is:inline>` + 各页 `:root{...}` 复制块 (Gen 2 短名 + cream 值) | 跟随架构重构每页抽到 `_<name>-css.ts` 时**同步删除 `:root` 块** + 改引 canonical token |
| 静态页 (about / privacy / compliance / 404 / index) | ⏳ 内联 `<style slot="head">` + 各页 `:root{...}` 复制块 (含 dual-theme media query 残留) | 同上 |
| `styles/canonical-tokens.css` | 🆕 待创建 | BaseLayout 必入口,§2.1 第一层 + 第二层全量声明 |
| [`styles/mobile-tokens.css`](../styles/mobile-tokens.css) | 🔴 DEPRECATED — 无 import,内容陈旧、命名(`--m-*`)已不用 | 删除或保留为归档,文件首加 DEPRECATED banner |
| `BaseLayout.astro` import token CSS | ❌ 未做 | `canonical-tokens.css` 创建后,frontmatter `import '../../styles/canonical-tokens.css'` |
| `scripts/check-design-tokens.cjs` | 🆕 Phase B 待建 | 解析 §2.1 表格 → 扫所有 `_*-css.ts` + `.astro` 的 `:root{}` → diff 一致性 → CI gate |

### 18.5 谁来执行

- **架构重构 PR**(把每个 hub 页 inline `<style>` 抽到 `_*-css.ts` 时):同 PR 内删该页 `:root{}` + 改引 canonical token
- **新增页面**:必须遵守 §18 归属规则,frontmatter import 已抽好的 section CSS,不在 `.astro` 内写整页 CSS
- **CI**:Phase B 的 `check-design-tokens.cjs` 自动守护 §2.1 ↔ `_*-css.ts` 一致性
- **本规范修改**:见 §14 修改本文件流程,视觉 token 变更必须先动 §2.1,再回到 `_*-css.ts` / `canonical-tokens.css`(待建)同步;baseline 重刷一次
