...
```
conformant   7 / 10 surface
migrating    0 / 10
legacy       3 / 10
```

**step 5 完了。** canonical の見出し（step 2・全 839 ページ）、`/map`（step 3）、`doc` + `static`
（step 4）、`hub` + `sector`（step 5）がトークン化された。残るは `detail`・`misc`・`feature`。
次は step 6 `detail`（556 ページ・最大）。
