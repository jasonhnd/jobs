...
### Added — Three GA4 funnel events (job_search_submit, result_view, occupation_tile_click)

- **`job_search_submit`** — fires from the treemap search box ([index.html:2560](index.html:2560)) after the user pauses typing for 800 ms. Independent from the existing 100 ms filter debounce so each unique query reports once instead of once-per-keystroke. Backspacing to a previously tracked query does not re-fire. Params: `query` (truncated to 100 chars), `match_count`, `language`. Marked as Key Event in `analytics/spec.yaml` — Yaku's推广方案 KPI #1.
- **`result_view`** — injected into all 1,104 per-occupation pages (`ja/<id>.html` + `en/<id>.html`) by `scripts/build_occupations.py`. Fires synchronously on page load via the gtag stub queue, so the call is captured even before `gtag.js` finishes loading. Params: `occupation_id`, `ai_risk_score`, `risk_tier` (`high` >=7 / `mid` 5-6 / `low` <=4), `language`. Distinct from GA4's auto-`page_view` because the parameters are typed and queryable. Marked as Key Event — Yaku's推广方案 KPI #3.
- **`occupation_tile_click`** — fires from the three paths that open a per-occupation page from the treemap: `desktop_click` (canvas mousedown, [index.html:2680](index.html:2680)), `touch_tap` (mobile second-tap, [index.html:2613](index.html:2613)), and `keyboard` (Enter/Space, [index.html:2738](index.html:2738)). All routed through a single `fireTileClick(rec, source)` helper. Params: `occupation_id`, `occupation_name_ja`, `occupation_name_en`, `ai_risk_score`, `risk_tier`, `salary`, `tile_position_idx`, `source`, `language`. Marked as Key Event — closes the treemap → occupation-page funnel.
- **`analytics/spec.yaml` updated** with the three new events. The legacy `search_used` entry is marked DEPRECATED with a pointer to `job_search_submit` (it was specced but never implemented). `chip_click` (already implemented) promoted to `conversion: true` so it lights up as a Key Event after the next `setup-ga4.mjs` run — Yaku's推广方案 KPI #2.
- **Treemap `occUrl()` now correctly routes to `/ja/<id>` or `/en/<id>`** based on the active language, replacing the stale `/occ/<id>-<slug>` slug-builder that had survived a prior incomplete deploy. Saves a 301 redirect hop on every tile click and stops GA4 from logging the redirected URL as the entry page. The two doc-text occurrences of `/occ/<id>-<slug>` in [index.html:1545](index.html:1545) and [index.html:1624](index.html:1624) are also updated.

### Added — Dynamic per-occupation OG cards

- **`api/og.tsx` Vercel Edge Function** — renders a 1200×630 PNG card on demand for any `?id=<n>&lang=<ja|en>`. Layout: UNOFFICIAL badge + site mark on top, a risk-band-colored block (matching the per-occupation page palette) carrying the AI-risk number and `/ 10` sub-label, occupation name (primary language in accent color + secondary language muted) on the right, workforce + average annual salary on a bottom rule. Uses `@vercel/og` (satori → resvg) with a per-request Google Fonts `Noto Sans JP` subset — only the glyphs actually drawn are fetched, keeping cold-start font payload under ~50 KB.
- **All 1,104 per-occupation pages now reference the dynamic card** via `<meta property="og:image" content=".../api/og?id=<n>&lang=<lang>">` plus matching `twitter:image`, `og:image:type`, `og:image:alt`. The static `/og.png` remains the home-page card; the dynamic card is per-occupation only. Replaces the prior shared-`og.png`-for-every-page setup that was causing social-platform previews to show the same homepage hero for every shared occupation link.
- **Cache strategy** — `Cache-Control: public, max-age=86400, s-maxage=86400, stale-while-revalidate=604800`. First request to a unique `id` cold-starts (~200–500 ms); every subsequent fetch — including all social-platform re-scrapes — is a Vercel CDN hit. Editing `api/og.tsx` ships new artwork on the next deploy without regenerating any files; `data.json` updates (e.g. a re-scoring pass) propagate the same way. No 552-PNG bake step.
- **`robots.txt` opens `/api/og`** explicitly while keeping `/api/` disallowed for the write-side `subscribe` / `feedback` endpoints. Most social platforms ignore robots.txt for OG scraping anyway, but this makes the intent legible to compliant crawlers.
- **`package.json` added at repo root** solely so Vercel detects the `@vercel/og` dependency and bundles the Edge Function. Python remains the build pipeline; Node is here only to enable the runtime.

### Changed — Per-occupation page URL scheme + bilingual split

- **Pure-numeric URLs.** Per-occupation pages move from `/occ/<id>-<slug>` (e.g. `/occ/428-general-office-clerk`) to `/ja/<id>` and `/en/<id>` (e.g. `/ja/428`, `/en/428`). Eliminates slug-rot risk when occupation names change and removes a class of broken-link bugs the prior scheme was prone to. Old slug URLs 301-redirect to `/ja/<id>` (or `/en/<id>` if the request carries `?lang=en`) via four `vercel.json` rules so already-indexed inbound links retain their authority.
- **Single-language pages, no in-page toggle.** Each occupation now ships as two separate static files — `ja/<id>.html` and `en/<id>.html` — instead of one bilingual file with a JS toggle. JA pages contain only Japanese DOM; EN pages contain only English. Schema.org `inLanguage` is now a single language code (was an array), `og:locale` and `<html lang>` are single-valued, and a small visible JA↔EN switch in the page header links the two with `rel="alternate"`. Net effect: cleaner crawler signals, no client-side language flicker, and proper hreflang pairing instead of language-mixed bodies.
- **Related-occupations block.** Every occupation page now ends with a 5-link "類似する職業 / Related occupations" section: 3 same-risk-band peers (±1 score, ranked by ID distance) + 2 ID-neighbors. Deterministic ordering — identical inputs always produce identical link sets. Builds internal SEO link equity (each page now has ~5 inbound internal links) and gives readers something to click after the stats.
- **Treemap tile click in `index.html`** now routes to `/ja/<id>` or `/en/<id>` depending on the active language toggle, replacing the previous slug-builder JS.
...
- **Vibrant light-mode treemap palette.** New `greenRedCSSLight()` function with deep, saturated anchors — emerald `(15, 195, 105)` → burnt orange `(235, 115, 25)` → vivid red `(235, 40, 55)` — so contrast survives alpha-blending on a `#fafafa` background. Light-mode tile alpha bumped from `0.62` to `0.95` (hover `1.0`) to prevent the white-wash effect that softer palettes suffered from. Dark-mode palette and alpha (`0.62` / `0.92`) unchanged. Theme switching now triggers a canvas redraw so palette + bg refresh in place — no reload needed.
- **Taller desktop treemap.** Desktop canvas height bumped from `w × 0.6` to `w × 1.05`, giving the smaller 552-tile cells enough vertical room to render their occupation labels. Mobile keeps the existing portrait `w × 2.6` ratio.
- **Larger tooltip on desktop.** Padding `12 / 14 → 14 / 16 px`, font `0.82 → 0.92 rem`, line-height `1.5 → 1.55`, max-width `360 → 400 px`. Title font `0.95 → 1.06 rem`, color changed from hard-coded `#fff` to `var(--fg)` so it reads correctly in both themes.

### Added — Design system

- **`Design.md` — the visual single source of truth.** A 400-line spec covering design tokens (light/dark colors, type scale, spacing, radius, shadows), the 3-state theme system, responsive breakpoints (`≥768 / ≤768 / ≤540 / ≤480 / ≤360`), treemap rendering rules (palette anchors, alpha by state, height ratio, label visibility thresholds), tooltip behavior (desktop hover-mode vs. mobile touch-mode), per-component standards (top-banner, stats-panel, share-btn, etc.), per-breakpoint mobile rules, and a §15 revision-history table. Going forward, every visual / responsive change updates `Design.md` first and the code is synced to match — `Design.md` wins on conflict.

### Added — Per-occupation pages (SEO)

- **552 static occupation detail pages — JA and EN, 1,104 HTML files total.** Each page (`/ja/<id>.html`, `/en/<id>.html`) is a hand-shaped occupation profile: heading, AI risk score with rationale, salary / age / hours / recruit-ratio / education breakdowns, and a back-to-treemap link. Every page carries the same 4 analytics scripts as the home page (Cloudflare WA, GA4, Vercel WA, Speed Insights), Schema.org `Occupation` JSON-LD, hreflang pairs, breadcrumbs. Generated by `scripts/build_occupations.py` from `data.json`.
- **`vercel.json` `cleanUrls` + redirects** — `/occ/<id>-<slug>` links resolve to `/ja/<id>` (or `/en/<id>` when the user is on EN). Treemap tile clicks now navigate to these per-occupation pages instead of MHLW jobtag's external site, keeping users on `mirai-shigoto.com` and crediting the site with the full session.
- **Sitemap expanded** to include all 1,104 occupation pages with proper `<xhtml:link rel="alternate" hreflang>` pairs.

### Added — Backend (D4 in OPC plan)

- **`api/subscribe.js`** — Vercel Edge Function (Tokyo edge, runtime `edge`) that accepts an email + occupation_id and adds the email to a Resend audience. Two audiences for routing: `audience_ja` and `audience_en`. Auth is via `RESEND_API_KEY` (full-access key, set in Vercel env). Returns `{ ok: true }` on success.
- **`api/feedback.js`** — sister Edge Function for the bottom-of-page feedback form (B2C / B2B segmentation). Records selected options + free-text + optional email. Sends a notification email to operator inbox via Resend transactional API.
- Both endpoints validate input (email regex, length caps), set CORS headers, and return JSON errors with semantic `error_reason` codes (`invalid_email` / `server_error` / `network`) so the front end can map them to GA4 `email_submit_*` events.

### Added — Social sharing

- **Footer share buttons** — six round 32×32 (mobile 36×36) icons: X (Twitter), LINE, Hatena Bookmark, LinkedIn, Copy Link, native Web Share API (mobile only). Each button hover-styled with the platform's brand color. Replaces an earlier bottom-of-page newsletter card that user-tested poorly. Share text and URL pre-filled per language. Click events tracked as GA4 `share_x_click` etc. with platform parameter.

### Added — SEO health monitoring

- **`scripts/seo-check.sh`** — daily probe that checks `mirai-shigoto.com` for live status, Cloudflare cache headers, sitemap reachability, robots.txt syntax, hreflang pair validity, JSON-LD presence on the home + privacy + a sample of occupation pages, OG card freshness, and CDN edge HIT rate. Outputs a pass/fail summary suitable for cron'ing and emailing on regression.
- One round of remediation against initial findings: stale OG card image, hreflang pair asymmetry on `/privacy`, missing `<meta name="robots">` self-declaration on a few occupation pages.

### Performance

- **A-tier perf passes** — preload `data.json` (`<link rel="preload" as="fetch" crossorigin>`), defer the 552-item screen-reader fallback list to `requestIdleCallback` so it never blocks initial paint, defer the GTM script to `loading="lazy"`-equivalent placement. `data.json` was responsible for the LCP critical path, now hits the canvas as soon as the 4 KB above-the-fold CSS finishes parsing.
- Speed Insights confirms: LCP < 1.6 s on 4G, INP < 80 ms on a Pixel-4-equivalent profile, CLS = 0 (was 0.27 pre-v0.5.0 reservation fix). Bundle stays under the 80 KB microsite budget per the project's web/performance rules.

### Fixed

- **保育士 (childcare worker) score corrected: 4 → 3.** The visualization was reading a stale `data.json` value. Also corrected high-risk-share aggregate: 31 % → 34 %. Numbers now match the published `data.json`.
- **SEO findings cleanup** — fixed all warnings surfaced by `scripts/seo-check.sh` first run.
...
- Vercel auto-deploy from `main` continues to pick up every push within ~30 s. Production validation post-deploy: real-Chrome navigation to `https://mirai-shigoto.com/`, dark→light toggle live (no reload), pixel-sample `rgb(236, 60, 62)` red + `rgb(78, 179, 94)` green confirmed, tooltip computed style `padding 14px 16px / 0.92 rem / max-width 400 px` confirmed, mobile 375 × 812 viewport `isMobile=true` + treemap aspect 2.601 + stats-panel 2-column + tooltip touch-mode CSS contracts all green.
