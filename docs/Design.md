...
| `--ink-3`        | `#8a7a6a` | 三次文字、breadcrumb、副情報、tc-meta（**≥18px または ≥14px+700w でのみ使う**、§13.6 を参照） |
| `--ink-meta`     | `#695745` | **NEW (RA-008, 2026-05-18)** — 11-13px の stat label / breadcrumb meta 等の小字号で `--ink-3` の代わりに使う AA 4.5:1 確保のための濃色派生。`dl.stats dt`、`.crumb-meta` 等 |
...
> **本ファイルから移出済み**。モバイルレスポンシブ規則全套（≤768 / ≤480 / ≤360 / ≤540 各段階詳細）は [Design-Mobile.md §3](./Design-Mobile.md#3-モバイル自適規則まとめ) 参照。本ファイル §4 はブレークポイントの **定義** のみを保持（768 / 480 / 360 / 540 閾値）、各段階で具体的に何をするかは Design-Mobile.md に委ねる。
...
| 2026-05-18 | §7 (新 Q&A details), §7 (新 interactive compare), index-source, MobileNav, Hub.ts, compare/index.astro | **RA-015 / RA-016 / RA-017 UI/UX 監査 P3 残り三項目** (1) **RA-015 lite** (P3-2 軽量版): モバイル専用 `<h2>` (`.mobile-hero-title` / `.m-top10-title` / `.m-map-preview-title` / `.m-map-title`) を `<p role="heading" aria-level="2">` に変更。Desktop 上 `display:none` で隠された見出しが outline-tool / source-grep の見かけ上の重複ノイズを生まないように。Visual / AT の挙動は変わらず（display:none 要素は AT に読み上げられない、役割重複も heading outline に出ない）。完全な desktop / mobile DOM 分離 (P3-2 フル版、Vercel Edge Middleware UA 分割) は別 PR — 本変更は最小コストで「outline 上の見かけ上の重複」を消す。(2) **RA-016** (P3-5): `/ja/q` hub の質問カードを `<details>` 折畳みに変更（旧: クリックで詳細ページに遷移するカード、新: その場で短い回答を展開、「詳しく見る →」リンクで詳細ページへ）。サイト内 Q&A の interaction model を統一（詳細ページ既存の `<details>` と揃える）。CSS `.qa-list / .qa-item details / details[open] / summary::after` 追加（`+` ↔ `×` icon toggle）。(3) **RA-017** (P3-3): `/ja/compare` index に **interactive 2-occupation comparison tool** 追加。client-side JS が `/data.search.json` (552 entries, ~90 KB) を fetch して datalist 構築、ユーザーが datalist から 2 つ選択 → `/data.detail/<padded id>.json` × 2 を Promise.all で fetch → side-by-side table を inline 描画。URL state は `?a=<id>&b=<id>` に mirror、`history.replaceState` で deep-link 可能、ページ load 時に URL params から自動復元。20 件の予設比較カードは下に温存（即時アクセス + 教育用途）。Inline script は static body（compute-csp-hashes が自動 hash 化、CSP 'unsafe-inline' 維持不要）。Build: 822 ページ、918 tests pass、CSP 16 sha256 hashes |
| 2026-05-18 | §2.1, §13, BaseLayout, canonical-css, Footer, vercel.json, 30+ surfaces | **RA-001…RA-014 UI/UX 監査 P0/P1/P2/P3 一括** Chrome MCP 三周回 + モバイル iframe 検査で見つけた 15+ 件問題を一括解消。**P0/P1**：(RA-001) BaseLayout に `<main id="main-content">` landmark + skip-link `本文へスキップ` を全 821 ページに導入、`canonical-css.ts` に `.skip-link:focus { left:12px; top:12px }` と `:focus-visible { outline: 2px solid var(--orange) }` 全局ルール追加。`canonical/hub.ts` / `sector.ts` / `static.ts` / `detail.ts` の重複 skip-link スタイルを削除して single source へ集約。(RA-003) `src/site/config.ts` に `OCCUPATION_COUNT.SCORED=552` / `TOTAL=556` 単一 source-of-truth を追加、`/ja/sectors` H1 + `/ja/rankings` H1 + 404 ページ + Ranking template seoDesc を `552` に統一（旧 `556` 散在は user-facing surfaces で全て removal）。`/about/data-sources` / `/about/glossary` のプロビナンス記述は `TOTAL=556` を保持。(RA-005) Footer.astro + `_index-bindings.ts` で share button 5 個の `href="#"` を実際の Twitter Intent / LINE Share / Hatena entry / LinkedIn / Facebook URL に置換（JS-disabled fallback 確保）。(RA-006) `_index-css.ts` の `.m-top10` に `overflow-x: clip`、`canonical-css.ts` の body に `overflow-x: clip` 二重防御 → モバイル iPhone 視口 (375px) で document scrollW=375 (was 383、8px 漏れ修正)。(RA-007) `_StatsGrid.astro` の em-dash を `<span aria-label="データなし">` 包装、`_id-css.ts` で `.stat-empty` 視覚降格 (color: --ink-3, weight 400)。**P2**：(RA-008) §2.1 に **新 token `--ink-meta: #695745`** 追加（`--ink-3` は WCAG AA を 11-13px で割るため）、`_id-css.ts dl.stats dt` を `--ink-3` → `--ink-meta` + font 0.7rem → 0.78rem で AA 4.5:1 確保。(RA-009) footer-links の `padding: 5px 14px` → `8px 14px` + `min-height: 28px` で WCAG 2.5.8 (24px) クリア。(RA-013) `BaseLayout.astro` に Cookie consent banner DOM + JS 追加、GA4 consent mode v2 をデフォルト `denied` → ユーザー同意で `granted` に切替。`canonical-css.ts` に banner CSS、CSP hash は `compute-csp-hashes.cjs` が自動更新。(RA-014) `searchInputDesktop` / `searchInputMobile` に `role="combobox"` + `aria-autocomplete="list"` + `aria-controls` + `aria-expanded` 追加、`attachSuggest()` の `render()` で開閉同期。**P3**：(RA-010) vercel.json に `/ja` → `/` と `/ja/` → `/` 永続 redirect 追加（旧 404）。(RA-011) BaseLayout + index-source.html に `<link rel="alternate" hreflang="ja">` + `x-default` 宣言。(RA-012) `/map` の `<main>` を `<article aria-label="職業マップ">` に変更（BaseLayout の `<main>` 内 nested を回避）。drawer の `<h3 class="mob-drawer-title">` を `<p role="heading" aria-level="2">` に変更（隠し H3 が page outline を汚染しない）。Build 通過、822 ページ、CSP 15 hashes、`check-rendered-leaks` OK |
