...
後続 issue（mms-6c / 6d / 6e / 6g）は次の文字列を**逐字使用**する。プレースホルダ `{N}` `{X.X}` `{日付}` だけを実行時に埋める。免責語彙「独自分析（非公式）」は合規スロットのため残す。C 向け文にモデル型番（Claude / GPT / Opus / Fable / Grok 等）を入れない。
...
### 切替 release の站内更新説明ドラフト（mms-6g で使用）
