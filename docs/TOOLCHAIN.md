...
Recorded **as of 2026-08-24** (preview deploy `dpl_FvJ3KQaLxHZekpiPBy4uV7QDLsVU`, `jobs-cdzvt3etz-zkscio.vercel.app`). Re-read Vercel Install logs and Function sizes when bumping Bun or `@vercel/og`.
...
| `@vercel/edge` | **1.3.1** | same | `middleware.ts` |
| React | **19.2.7** (OG `createElement` only; no `@astrojs/react`, no client React) | same | inside the `api/og` Edge bundle |
| Playwright / axe | **1.61.1** / **4.12.1** (exact pins, no `^`) | **not executed** | npm packages may install as devDependencies; **Chromium is not installed**; e2e is not in `buildCommand` |
...
| `@vercel/og` on Edge vs Node | Platform OG docs (2026-06-16) lead with **Node.js**. npm `@vercel/og@1.0.1` README still says Node **and** Edge. **1.0 on Edge is unproven until a preview Function boots.** | [OG image generation](https://vercel.com/docs/og-image-generation) (2026-06-16) |
...
| Local `bun run test:e2e` | Chromium against `dist-astro/` via `scripts/e2e-server.cjs` | CI/Vercel |
